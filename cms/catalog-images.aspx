<%@ Page Title="" Language="C#" MasterPageFile="~/cms/admin.master" AutoEventWireup="true" CodeBehind="catalog-images.aspx.cs" Inherits="ProductivityPointGlobal.cms.catalog_images" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
	<div class="row">
		<div class="col-md-12">
			<h3>Manage Catalog Images</h3>
		</div>
	</div>

	<asp:Button ID="btnNew" runat="server" CssClass="btn btn-success" Text="Add New" OnClick="btnNew_Click" />

	<asp:GridView ID="ctlCMSCatalogImages" DataKeyNames="id" AutoGenerateColumns="False" GridLines="None" AllowPaging="False" CssClass="table table-hover" runat="server" OnRowCommand="ctlCMSCatalogImages_RowCommand">
		<Columns>
				...
				<ItemTemplate>
					<td>
						<div class="btn-group" role="group" aria-label="...">
							<asp:Button Text="Edit" CssClass="btn btn-warning" ID ="btnEdit" CommandName="ImgEdit" CommandArgument='<%# Bind("id") %>' runat="server" />
							<asp:Button Text="Delete" CssClass="btn btn-danger" ID="btnDelete" CommandName="ImgDelete" CommandArgument='<%# Bind("id") %>' OnClientClick='<%# Eval("catalog_name", "return confirm(\"Delete item [{0}]?\");") %>' runat="server" />
						</div>
					</td>
					<td>
						<asp:Label ID="ctlCategory" Text='<%# Eval("catalog_name") %>' runat="server" />
					</td>
					<td>
	...
	<asp:Panel ID="pnlModalOverlay" runat="server" ClientIDMode="Static" Visible="false">
		<asp:Panel ID="pnlModal" runat="server" ClientIDMode="Static">
			...
			<table>
				<tr>
					<td><asp:Label ID="lblName" runat="server" Text="Catalog Name"></asp:Label></td>
					<%--<td><asp:TextBox ID="tbName" runat="server"></asp:TextBox></td>--%>
					<td><asp:DropDownList id="ddName" runat="server" DataValueField="catalog_name" DataTextField="catalog_name"></asp:DropDownList></td>
				</tr>
				<tr>
