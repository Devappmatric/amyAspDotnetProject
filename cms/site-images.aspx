<%@ Page Language="C#" MasterPageFile="~/cms/admin.master" AutoEventWireup="true" CodeBehind="site-images.aspx.cs" Inherits="ProductivityPointGlobal.cms.site_images" %>
...
<%--							<asp:Button Text="Delete" CssClass="btn btn-danger" ID="btnDelete" CommandName="ImgDelete" CommandArgument='<%# Bind("id") %>' OnClientClick='<%# Eval("page_name", "return confirm(\"Delete item [{0}]?\");") %>' runat="server" />--%>
						</div>
					</td>
					<td>
						<asp:Label ID="ctlPage" Text='<%# Eval("page_name") %>' runat="server" />
					</td>
					<td>
		...
		<asp:Panel ID="pnlModal" runat="server" ClientIDMode="Static">
			...
			<table>
				<tr>
					...
					<td><asp:DropDownList id="ddName" runat="server" DataValueField="page_name" DataTextField="page_name"></asp:DropDownList></td>
				</tr>
				<tr>
