...
namespace ProductivityPointGlobal.Cart
{
    public partial class Receipt : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                var orderId = 0;// (int)Session["orderid"];
                if (Session["orderid"] != null)
                {
                    orderId = (int)Session["orderid"];
                    ltrOrderId.Text = orderId.ToString();
                ...
                //ctlOrderNumber.Text = orderId.ToString();
        ...
        private void OrderInfo(int orderId)
        {
            using(var cnn=new SqlConnection(ConfigurationManager.ConnectionStrings["SQLConnectionProd"].ConnectionString))
            {
                ...
                using(var r = cmd.ExecuteReader(CommandBehavior.CloseConnection))
                {
                    ...
                    {
                        ctlOrderNumber.Text = orderId.ToString();
                        ctlPaymentMethod.Text = (string)r["paymentMethod"];

                        //if (r["datePaid"] != DBNull.Value)
                            ctlTodaysDate.Text = ((DateTime)r["dateTime"]).ToString("M/d/yyyy");
                        ctlTotalCost.Text = ((decimal)r["amount"]).ToString("c");
                        lblCartTotal.Text = ((decimal)r["total"]).ToString("c");
                        if (r["purchaseOrder"] != DBNull.Value)
                            ctlPO.Text = (string)r["purchaseOrder"];
                        ltrUserAgent.Text = (string)r["userinfo"];
                        ctlInvoiceNumber.Text = (string)r["orderstatus"];
                        var discount = (decimal)r["discount"];
                        if (discount != 0)
                        {
                            ctlTotalCostReg.Text = ((decimal)r["subtotal"]).ToString("c");
                            ctlDiscountTotal.Text = discount.ToString("c");
        ...
        private void EmailUser()
        {
            ...
            foreach (var row3 in lvCart.Items)
            {
                ...
                var oClassTime = ((Literal)row3.FindControl("ltrEndTime")).Text;
                ...
                mailMessage.Append("<strong>Time:</strong> " +  oClassTime + "<br />");
            ...
            foreach(var row in lvStud.Items)
            {
                var sStudent = ((Literal)row.FindControl("ltrName")).Text;
                var sEmail = ((Literal)row.FindControl("ltrEmail")).Text;
                var sCourse = ((Literal)row.FindControl("ltrCourse")).Text;
            ...
            if (!string.IsNullOrEmpty(ctlPO.Text))
            {
                mailMessage.Append("<strong>Purchase Order #:</strong> ")
                    .Append(ctlPO.Text).Append("<br />");
            }
            ...
            mailMessage.Append("<strong>Order Status:</strong> " + ctlInvoiceNumber.Text + "<br />");
            mailMessage.Append("<strong>Total Cost:</strong> " + lblCartTotal.Text + "<br /><br />");
            ...
            foreach (GridViewRow row2 in ctlUserInfo.Rows)
            {
                ...
                UserFullName = uFirstName + " " + uLastName;
            ...
            //mailMessage.Append("<p style=\"font-size:small\"><strong>Order has been placed from: </strong>")
            //    .Append(ltrUserAgent.Text)
            //    .Append("</p>");
            ...
            {
                From = new System.Net.Mail.MailAddress("registrar@productivitypointls.com", "Productivity Point Learning Solutions"),
                Subject = "Productivity Point Learning Solutions - Customer Receipt/Purchase Confirmation",
                Body = mailMessage.ToString(),
                IsBodyHtml = true
            };
            msg.To.Add(new System.Net.Mail.MailAddress(EmailAddress, UserFullName));
            msg.Bcc.Add(new System.Net.Mail.MailAddress(ConfigurationManager.AppSettings["OrderMailTo"], "Sales Admin"));
            try
            {
                App.SendEmail(msg);
                Session.Remove("orderid");
